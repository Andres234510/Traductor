...
// Función para mostrar el diccionario inicial
...
// Traductor: buscar y mostrar traducción
translateBtn.addEventListener('click', () => {
    const inputWord = wordInput.value.trim().toLowerCase();
    const selectedLanguage = [...languageSelection].find(radio => radio.checked)?.value;

    if (!inputWord || !selectedLanguage) {
        translationResult.textContent = 'Por favor, ingrese una palabra y seleccione un idioma.';
        translationResult.classList.remove('hidden');
        return;
    }

    const allWords = getAllWords();
    const word = selectedLanguage === 'en-es'
        ? allWords.find(w => w.english.toLowerCase() === inputWord)
        : allWords.find(w => w.spanish.toLowerCase() === inputWord);

    if (word) {
        const translation = selectedLanguage === 'en-es' ? word.spanish : word.english;
        translationResult.textContent = `Traducción: ${translation}`;
    } else {
        translationResult.textContent = `Palabra no encontrada en el diccionario (${selectedLanguage === 'en-es' ? 'Inglés a Español' : 'Español a Inglés'}).`;
    }
    translationResult.classList.remove('hidden');
});
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const sortSpanishBtn = document.getElementById('sort-spanish-btn');
    const dictionaryTable = document.querySelector('#dictionary-table tbody');

    const getAllWords = () => Object.values(dictionary.categories).flat();

    const renderDictionary = (category = 'all', sortField = null) => {
        let words = category === 'all' ? getAllWords() : dictionary.categories[category];

        if (sortField) {
            words = words.sort((a, b) => a[sortField].localeCompare(b[sortField]));
        }
    ...
    // Ordenar por inglés
    sortEnglishBtn.addEventListener('click', () => {
        renderDictionary(categoryFilter.value, 'english');
    });

    // Ordenar por español
    sortSpanishBtn.addEventListener('click', () => {
        renderDictionary(categoryFilter.value, 'spanish');
    });
    ...
    // Renderizar la tabla por defecto
    renderDictionary();
});
...
addWordForm.addEventListener('submit', (e) => {
    ...
    const newWordEs = document.getElementById('new-word-es').value.trim();
    const newWordExample = document.getElementById('new-word-example').value.trim();
    const newWordCategory = document.getElementById('new-word-category').value;
    ...
    const categoryWords = dictionary.categories[newWordCategory];
    if (categoryWords.some(w => w.english.toLowerCase() === newWordEn.toLowerCase() || w.spanish.toLowerCase() === newWordEs.toLowerCase())) {
        alert('Esta palabra ya existe en la categoría seleccionada.');
        return;
    }
    ...
    categoryWords.push(newWord);
    alert('Nueva palabra añadida con éxito.');
    addWordForm.reset();

    if (categoryFilter.value === 'all' || categoryFilter.value === newWordCategory) {
        renderDictionary(categoryFilter.value === 'all' ? getAllWords() : dictionary.categories[newWordCategory]);
    }
});


// Inicializar la tabla del diccionario
renderDictionary(getAllWords());
